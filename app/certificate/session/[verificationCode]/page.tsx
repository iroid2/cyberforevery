...
import { getCertificateRequestByVerificationCode } from "@/app/actions/certificates";

export default async function SessionCertificatePage({
  params,
}: {
  params: Promise<{ verificationCode: string }>;
}) {
  const { verificationCode } = await params;
  const certRequest = await getCertificateRequestByVerificationCode(verificationCode);

  if (!certRequest) {
    notFound();
  }

  const { student, session, submission } = certRequest;
  const learnerName = student.name;
        ...
        <section className="rounded-[2rem] border border-white/10 bg-white/5 p-6 shadow-[0_25px_90px_-60px_rgba(191,255,0,0.45)] backdrop-blur md:p-10">
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div>
              <p className="text-[11px] font-bold uppercase tracking-[0.32em] text-[#bfff00]">
                Verified Session Certificate
              </p>
              <h1 className="mt-4 text-3xl font-extrabold tracking-tight md:text-5xl">
                {courseTitle}
              </h1>
              <p className="mt-4 max-w-2xl text-sm leading-7 text-white/75 md:text-base">
                This certificate confirms that <strong>{learnerName}</strong> successfully completed the learning requirements for the <strong>{session.course.subject}</strong> session held on{" "}
                {formattedDate} with a score of <strong>{scorePercent}%</strong>.
              </p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-sm">
              <div className="font-semibold text-white">Session Date</div>
              <div className="mt-1 text-white/70">{formattedDate}</div>
            </div>
          </div>

          <div className="mt-8 grid gap-4 md:grid-cols-3">
            <Metric label="Learner" value={learnerName} />
            <Metric label="Score" value={`${submission.score}/${submission.totalQuestions} (${scorePercent}%)`} />
          ...
          <div className="mt-8 rounded-[1.5rem] border border-[#bfff00]/20 bg-[#bfff00]/10 p-6">
            ...
            <p className="mt-3 break-all text-lg font-semibold tracking-[0.08em] text-white">
              {certRequest.verificationCode}
            </p>
            <p className="mt-3 text-sm leading-6 text-white/70">
              Issued by cyber4every1 and linked directly to the platform session record for audit and verification.
            </p>
          </div>
